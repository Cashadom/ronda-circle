...
export async function leaveCircle(circleId, userId) {
  if (!circleId || !userId) throw new Error('Missing required parameters')
  ...
  await deleteDoc(doc(db, 'users', userId, 'circles', circleId))

  await updateDoc(doc(db, 'circles', circleId), {
    members_count: Math.max(0, Number(circle.members_count || 0) - 1),
    updated_at: serverTimestamp(),
  })
}
...
// ─── getCircleLatestMoments ───────────────────────────────────────────────
export async function getCircleLatestMoments(circleId, limitCount = 6) {
  if (!circleId) return []
